...
class Qwen3VLEmbedder():
    def __init__(
        ...
        **kwargs
        ...
        trust = hf_kwargs.pop("trust_remote_code", True)
        self.model = Qwen3VLForEmbedding.from_pretrained(
            model_name_or_path, trust_remote_code=trust, **hf_kwargs
        ).to(device_t)
        self.processor = Qwen3VLProcessor.from_pretrained(
            model_name_or_path, padding_side="right", trust_remote_code=trust
        )
        self.model.eval()
    ...
    def _preprocess_inputs(self, conversations: List[List[Dict]]) -> Dict[str, torch.Tensor]:
        ...
        # 含图/视频时序列很长（大量 image token）；对 max_text_length 做 truncation
        # 会破坏占位符与 input_ids 一致，触发 Qwen3VLProcessor 校验错误。
        multimodal = bool(images) or bool(videos)
