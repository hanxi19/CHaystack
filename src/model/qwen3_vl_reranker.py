# Vendored from https://huggingface.co/Qwen/Qwen3-VL-Reranker-8B (scripts/qwen3_vl_reranker.py)
# with: optional device kwarg; fix video frame sampling (sample_frames).
...
class Qwen3VLReranker:
    ...
    def tokenize(self, pairs: list, **kwargs):
        ...
        for i, ele in enumerate(batch_inputs["input_ids"]):
            batch_inputs["input_ids"][i] = self.truncate_tokens_optimized(
                batch_inputs["input_ids"][i][:-5],
                max_length,
                self.processor.tokenizer.all_special_ids,
            ) + batch_inputs["input_ids"][i][-5:]
        # 只 pad 到 batch 内最长；勿传 max_length 以免触发「无 truncation 时忽略 max_length」的告警。
        ...
        for key in temp_inputs:
            batch_inputs[key] = temp_inputs[key]

        # tokenizer.pad 会拉长 input_ids / attention_mask，但沿用 processor 里旧的 mm_token_type_ids
        # 会导致与 attention_mask 长度不一致，进而在 Qwen3-VL get_rope_index 里布尔索引报错。
        ids = batch_inputs["input_ids"]
        if isinstance(ids, torch.Tensor):
            mm_lists = self.processor.create_mm_token_type_ids(ids.tolist())
            batch_inputs["mm_token_type_ids"] = torch.tensor(
                mm_lists, dtype=torch.long, device=ids.device
            )
        else:
            batch_inputs["mm_token_type_ids"] = self.processor.create_mm_token_type_ids(
                ids
            )
